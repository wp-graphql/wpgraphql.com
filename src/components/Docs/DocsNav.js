            ...
            <div key={key}>
              ...
              <ul className="mb-6 space-y-6 lg:space-y-2 border-l border-slate-100 dark:border-slate-800">
                {children.map((child) => {
                  return (
                    <li key={child.href}>
                      <Link href={child.href} className="block border-l pl-4 -ml-px border-transparent hover:border-slate-400 dark:hover:border-slate-500 text-slate-700 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-300">
                          {child.title}
                      </Link>
                    </li>
                  )
